...
from src.speechsplit_utils import quantize_f0_torch
from src.prenet import ConvNorm, LinearNorm
...
class Transformer(TTSInterface, torch.nn.Module):
    ...
    def __init__(self,args):
        ...
        self.dim_con=args.prenet.dim_con
        self.dim_enc=args.prenet.dim_enc
        self.chs_grp=args.prenet.chs_grp
        self.dim_pit=args.prenet.dim_pit
        self.dim_enc_3=args.prenet.dim_enc_3
        self.dim_rhy=args.prenet.dim_rhy
        ...
        if self.spk_embed_dim is not None and self.whereusespkd == 'withprosody':
            self.prosody_projection = torch.nn.Linear(
                args.adim*2 + self.spk_embed_dim, args.adim
            )
        else:
            self.prosody_projection = torch.nn.Linear(
    ...
    def forward(self, xs, x_pitch, x_rhythm, ilens, ys, olens, spembs=None, *args, **kwargs):
        ...
        """
        # print(f'xs : {xs.shape} \nx_pitch : {x_pitch.shape} \nx_rhythm : {x_rhythm.shape} \nys: {ys.shape} \nilens : {ilens.shape}\nolens : {olens.shape} \n spembs :{spembs.shape}')
        ...
        content_pitch_input = torch.cat((xs_ds, x_pitch), dim=-1) # [B, T, F+1]
        content_pitch_input_intrp = self.interp(content_pitch_input) # [B, T, F+1]
        ...
        content_pitch_input_intrp = content_pitch_input_intrp.transpose(2,1)
        x = content_pitch_input_intrp[:, :self.dim_con, :]
        f0 = content_pitch_input_intrp[:, self.dim_con:, :]

        # print(f'x : {x.shape} \nf0 : {f0.shape}')

        for conv_1, conv_2 in zip(self.convolutions_content, self.convolutions_f0):
            x = F.relu(conv_1(x))
            f0 = F.relu(conv_2(f0))
            x_f0 = torch.cat((x, f0), dim=1).transpose(1, 2)
            
            x_f0 = self.interp(x_f0)
        ...
        x_2 = x_rhythm.transpose(2,1)
        for conv in self.convolutions_rhythm:
            x_2 = F.relu(conv(x_2))
    ...
    def inference(self, x, x_pitch, x_rhythm, spemb=None, *args, **kwargs):
        ...
        x_pitch = x_pitch.unsqueeze(0)
        x_pitch = x_pitch.unsqueeze(-1)
        content_pitch_input = torch.cat((x_ds, x_pitch), dim=-1) # [B, T, F+1]
        #content_pitch_input_intrp = self.interp(content_pitch_input) # [B, T, F+1]
        ...
        content_pitch_input = content_pitch_input.transpose(2,1)
        x = content_pitch_input[:, :self.dim_con, :]
        f0 = content_pitch_input[:, self.dim_con:, :]
        ...
        #print(f'x : {x.shape} \n x_2 : {x_2.shape} \n f0 : {f0.shape}')
        x_ds = torch.cat((x.transpose(1, 2), x_2, f0.transpose(1, 2)), dim=-1)
        x_ds = self.encoder_projection(x_ds)
    ...
    def _integrate_with_prosody_embed(self, hs, spembs, prosody_vec, prosody_vec_mask):
        prosody_vec, _ = self.prosody_encoder(prosody_vec, None)
        prosody_vec = self.prosody_bottleneck(prosody_vec.transpose(1, 2)).transpose(1, 2)
        ...
        hs = self.prosody_projection(torch.cat([hs, prosody_vec_att, spembs], dim=-1))
